...
>
> ⚠️ **brain 可插拔（实跑现状）**：下文"Qwen"= **brain 角色**，当前实跑为 **nemotron-3-super:120b**（Ollama, reasoning 模型）；端点 `:8001/:8002/:8003` 为目标态,当前实跑为 brain `:11434` + gemma `:11436`/`:11437`（见 03 §8 / 00 §0.0）。**设计语义不变**。

---

## 0. 真实比赛统计基准 + 引擎物理覆盖（模拟按此对标 · 2026-06 代码核实）

> 目的:确保跑出**一场逼真比赛**——球员跑位/动作/跳跃/体能 + 比赛级统计(回合/射门/控球…)都对标**真实均值**。①引擎到底模拟了什么(可行性核实)②真实比赛统计基准(模拟目标)③怎么按它驱动 + 校验。

### 0.1 引擎物理覆盖（代码核实:能做 / 不能做）

| 维度 | 建模? | 代码依据 / 说明 |
|---|---|---|
| 跑位移动 | ✅ | `getRunMovement`:每拍朝 `intentPOS`(`formationCheck`)走 **±1 单位/轴**;球近(±60)朝球走 |
| 冲刺 | ✅ ~2× | `getSprintMovement`:**±2 单位/轴**(持球冲刺 ±4),比 run 快一倍 |
| 体能/累 | ✅ | run `fitness-=0.005`/拍、sprint `-=0.01`/拍(冲刺更累),到阈值(20/30)不再降 → 累了跑不动 |
| 跳跃/争顶/头球 | ✅ | `calcHeight = height + jumping` = **垂直触球高度**;球有 z 高度;够得着→`setAttackBodyPart` 判头球(头球射门 roll +3) |
| 扑救垂直够球 | ✅ | `checkGoalScored` 用 GK `height+jumping` 接近度判扑出(§4.4) |
| 边界 / 位置阵型 | ✅ | `completeMovement` 夹球场内;`currentPOS/originPOS/intentPOS` + formationCheck 维持阵型 |
| 射门/传球/抢断/越位/定位球/点球 | ✅ | 各引擎原函数(00 复用铁律) |
| ⚠️ 速度按 `pace` 个体化 | ❌ | 步长**固定**(run=1/sprint=2),**不随 pace 缩放**;快慢差异体现在"何时冲刺(决策)+ 动作结果(技术)",非原始步速。改它=破铁律→不做,接受此抽象 |
| ⚠️ 球员实体碰撞 / 圆柱体 | ❌ | **无**球员-球员碰撞/半径/圆柱体/身体卡位;球员是"带垂直够球高度的 2D 点",可重叠。对抗靠 `closestPlayer`+技术 roll 解算,**非物理挤撞**。竖直争顶有(height+jumping),**水平身体卡位没有** |

> **结论**:跑/冲刺/体能/争顶跳/站位/越位/定位球/射门/扑救/点球/抢断 **全部已建模,够跑一场逼真比赛**。两个已知抽象——速度不按 pace 缩放、无身体圆柱碰撞——是引擎设计取舍,可接受(要 3D 圆柱碰撞需改引擎,违反零改铁律)。

### 0.2 真实比赛统计基准（模拟目标 · 顶级男足/世界杯均值量级）

| 指标(每队/场) | 真实均值 | 区间 |
|---|---|---|
| 比赛时长 | 90′（淘汰赛平局 →+30′ 加时 → 点球大战，05 §3） | — |
| 控球率 | 50%（强弱差时一边可达 60–65） | 35–65% |
| 射门 / 射正 | ~13 / ~4–5（射正率 ~35%） | 8–18 / 33–45% |
| 进球 | ~1.4（全场总 ~2.8，泊松/DC 参照） | 0–4 |
| **危险进攻/回合**（你关注的"home 11 / away 9"量级）| ~10 | 8–14 |
| 总控球段（possession sequences）| ~110 | 90–140 |
| 传球 / 成功率 | ~450 / ~83% | 350–650 / 78–88% |
| 抢断 ~15 · 拦截 ~8 · 角球 ~5 · 越位 ~2 · 犯规 ~11 | — | — |
| 黄牌 ~1.8 · 红牌 ~0.1 · GK 扑救 ~3–4/场 · 扑救率 ~70% | — | 65–75% |

> **强弱对阵按实力差偏移**(强队控球↑/射门↑),由 MatchDirector 按两队 rating 差 skew(02 §1)。这些均值可后续用**你预测盘/真实 WC 数据**替换校准。

### 0.3 怎么"按真实值模拟"（驱动 + 校验闭环）

1. **赛前**:MatchDirector(brain) 按两队 rating/风格,在 §0.2 均值附近**采样一份 match config**(控球目标/tempo/directness/射门倾向),强弱差→偏移(02 §1)。
2. **赛中**:PossessionDirector 反馈把控球率拉向目标(§3.2);tempo/directness 引导回合数与射门频率(§3.3);`GEMMA_EVERY` + `ITER_PER_HALF` 是旋钮。
3. **`ITER_PER_HALF` = 校准旋钮**:引擎**不固定"一拍=多少秒"**(readme:用户自定每半场迭代数)→ 调它 + 决策密度,使**涌现统计落 §0.2 区间**;报告"分钟"≈ `iter / (ITER_PER_HALF/45)` 折算。
4. **赛后**:ReportAgent 统计射门/控球/回合/扑救… 对照 §0.2;**calibration gate**(05 §3.7 指标)——落区间才合格,否则调旋钮/权重(像预测盘校准)。
